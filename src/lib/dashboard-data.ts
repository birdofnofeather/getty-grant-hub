...
const posUsd = (r: CleanGrant) => (r.amountAwarded_USD > 0 ? r.amountAwarded_USD : 0);

export interface YearRow { year: number; count: number; usd: number; }

export function perYear(rows: CleanGrant[]): YearRow[] {
...
export function peopleVsOrgByYear(rows: CleanGrant[]): PeopleOrgRow[] {
  ...
  for (const r of rows) {
    ...
    if (isIndividualGrant(r)) { a.indCount++; a.indUsd += posUsd(r); }
    else { a.orgCount++; a.orgUsd += posUsd(r); }
  }
  return Array.from(m.values()).sort((a, b) => a.year - b.year);
}

export function topInitiatives(rows: CleanGrant[], by: 'usd' | 'count', limit = 12) {
